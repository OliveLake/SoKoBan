...
const int Game::YWall=10;
const int Game::Bomb=70;
const int Game::GOLD=90;
...
void Game::restart()
{
    ...
    mp[player.man.x][player.man.y]+=MAN;
    if(isyellow)mp[YellowP.x][YellowP.y]=YWall;
    if(isGold)mp[GoldP.x][GoldP.y]=GOLD;
    if(isBomb)mp[BombP.x][BombP.y]=Bomb;
    for(set<Point>::iterator i=player.box.begin();i!=player.box.end();i++)
       { mp[i->x][i->y]+=BOX;}
}
...
bool Game::isWin()
{
    for(set<Point>::iterator i=player.box.begin();i!=player.box.end();i++)
        ...
        {
            if(isyellow){
                if(YellowP.y==player.man.y&&YellowP.x==player.man.x){
                    return true;
                } else{return false;}
            }
            else{return false;}
...
void Game::move(int k)
{
    ...
    if(mp[p.x][p.y]==Bomb)
    {
        ...
        if(wall[pp.x][pp.y])
        {
            mp[p.x][p.y]-=Bomb;
            mp[pp.x][pp.y]-=WALL;
            wall[pp.x][pp.y]=false;
        }else if(mp[pp.x][pp.y]==GOLD)
        {
            mp[p.x][p.y]-=Bomb;
            mp[pp.x][pp.y]-=GOLD;
        }
        else if(mp[pp.x][pp.y]==BOX){p=player.man;}
        else {
              mp[p.x][p.y]-=Bomb;
            mp[pp.x][pp.y]+=Bomb;
        }
    }
    if(mp[p.x][p.y]==Bomb+AIM)
    {
        Point pp(p.x+mx[k],p.y+my[k]);
        if(wall[pp.x][pp.y])
        {
